...
async function handle(request) {
  ...
  // Latest release tag for the version link; omit the line if GitHub is unreachable.
  const tag = await fetch("https://api.github.com/repos/usually-frustrated/termer/releases/latest", {
    headers: { "Accept": "application/vnd.github+json", "User-Agent": "termer-install" },
    cf: { cacheTtl: 300 }
  }).then(r => r.json()).then(r => r.tag_name).catch(() => null)

  const version = tag
    ? `<a class="ver" href="https://github.com/usually-frustrated/termer/blob/main/CHANGELOG.md">${tag}</a>`
    : ""
...
<style>
  ...
  .ver { display: inline-block; margin-top: 10px; font-size: 12px; color: color-mix(in srgb, CanvasText 55%, Canvas); }
</style>
<main>
